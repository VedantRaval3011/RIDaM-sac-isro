  ...
  <div class="flex flex-col gap-5 rounded-3xl drop-shadow-md backdrop-blur box-border p-7 bg-gradient-to-r from-blue-100 to-blue-100 container white">
    ...
    <div class="flex gap-10 projection" >
      ...
      <div class="flex gap-2 boxes ">
       ...
       <div>
         <button class=" add-2 py-2 px-4 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 mr-56" v-show="!showProjection3 && showProjection2" @click="addInputTwos">
          Add
        </button>
       </div>
        <div class="third">
...
export default {
  props: ["subtheme", "theme","initialData","id"],
  ...
  setup(props, { emit }) {
    ...
    watch(() => props.initialData, (newVal, oldVal) => {
      if (newVal) {
        // Update the values of the form inputs with the initial data
        extension.value = newVal.extension;
        interleave.value = newVal.interleave;
        Compression.value = newVal.Compression;
        resampling.value = newVal.resampling;
        Projection.value = newVal.Projection
      }
    }, { immediate: true });

    if(props.initialData && props.initialData.id){
     CorrectProjectOne.value=true;
     CorrectProjectTwo.value=true;
     CorrectProjectThree.value=true;
    const selectedData = jsonData.find(item => item.id === props.initialData.id);
    if (selectedData) {
      // Accessing values from the selected data
      const jsonExtension = selectedData.extension;
      const jsonCompression = selectedData.compression;
      const jsonReSampling = selectedData.resampling; 
      const jsonInterleave = selectedData.interleave;
      const jsonProjection = selectedData.projection[0];
      const jsonProjection2 = selectedData.projection[1];
      const jsonProjection3 = selectedData.projection[2];
      // Setting initial values based on JSON data
      extension.value = jsonExtension;
      resampling.value = jsonReSampling;
      Compression.value = jsonCompression;
      interleave.value = jsonInterleave;
      Projection.value = jsonProjection;
      Projection2.value = jsonProjection2;
      Projection3.value = jsonProjection3;
      
      console.log("Compression," , jsonCompression);
    } else {
      console.error('Data for the provided id not found.');
    }
    }
  ...
  if (Projection3.value.trim() !== "") {
    ...
    // console.log("prop id",props.initialData.id);
