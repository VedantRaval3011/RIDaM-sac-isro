  ...
  setup(props, { emit }) {
    ...
    const Purpose = ref("");
     ...
     watch(() => props.initialData, (newVal, oldVal) => {
      if (newVal) {
        ...
        dataType.value = newVal.dataType;
        AccessConstraint.value = newVal.AccessConstraint;
        UseConstraints.value = newVal.UseConstraints;
        Purpose.value = newVal.Purpose;
        firstRowKeywords.value = newVal.firstRowKeywords;
        secondRowKeywords.value = newVal.secondRowKeywords;
      }
    }, { immediate: true });

    if(props.initialData && props.initialData.id){
     CorrectAccessConstraints.value = true; 
     CorrectPurpose.value = true; 
     CorrectUseConstraints.value = true; 
     CorrectPurpose.value = true; 
    //  CorrectAccessConstraints.value = true; 
    const selectedData = jsonData.find(item => item.id === props.initialData.id);
    if (selectedData) {
      // Accessing values from the selected data
      const jsonStatus = selectedData.metadata.status;
      const jsonEdition = selectedData.metadata.edition;
      const jsonDataType = selectedData.metadata.dataType; 
      const jsonAccessConstraints =selectedData.metadata.access_constraints;
      const jsonUseConstraints = selectedData.metadata.use_constraints;
      const jsonPurpose = selectedData.metadata.purpose;
      const jsonFirstRowKeywords = selectedData.metadata.firstRowKeywords;
      const jsonSecondRowKeywords = selectedData.metadata.secondRowKeywords;
      // Setting initial values based on JSON data
      status.value = jsonStatus;
      edition.value = jsonEdition;
      dataType.value = jsonDataType;
      AccessConstraint.value = jsonAccessConstraints;
      UseConstraints.value = jsonUseConstraints;
      Purpose.value = jsonPurpose;
      firstRowKeywords.value = jsonFirstRowKeywords;
      secondRowKeywords.value = jsonSecondRowKeywords;
      
      console.log("SecondRowKeywords:," , jsonSecondRowKeywords);
    } else {
    ...
    const handleRightClick = () => {
      ...
      emit("update-data", data);
        ...
        console.log('emiteed');
        console.log(firstRowKeywords.value);
