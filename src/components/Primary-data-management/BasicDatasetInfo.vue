  ...
  <div class=" flex flex-col gap-5 rounded-3xl drop-shadow-md backdrop-blur box-border p-7 bg-gradient-to-r from-blue-100 to-blue-100 container white">
    ...
    <div class="flex gap-14 theme">
      ...
      <select name="theme" id="theme" v-model="theme" class="themeInput h-10 rounded-3xl bg-blue-50 shadow-md border-none text-base p-2 pr-11 box-border align-middle">
        <option value="T1" :selected="theme === 'T1'">
          Generic
        </option>
        <option value="T2" :selected="theme === 'T2'" >vegitation</option>
        <option value="T3" :selected="theme === 'T3'">hydrology</option>
        <option value="T4" :selected="theme === 'T4'">environment</option>
    ...
    <div class="flex gap-5 subtheme">
      ...
      <select name="subtheme" class=" subthemeInput h-10 rounded-3xl bg-blue-50 shadow-md border-none text-base p-2  pr-8 pl-4 align-middle" v-model="subtheme">
        <option value="S1" :selected="subtheme === 'S1'" >Generic</option>
        <option value="S2" :selected="subtheme === 'S2'">subtheme1</option>
        <option value="S3" :selected="subtheme === 'S3'">subtheme2</option>
        <option value="S4" :selected="subtheme === 'S4'">subtheme3</option>
      </select>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function validatesourcelocation() {
     ...
     watch(() => props.initialData, (newVal, oldVal) => {
      if (newVal) {
        ...
        subtheme.value = newVal.subtheme;
        sourcelocation.value = newVal.sourcelocation;
   ...
   if(props.initialData && props.initialData.id){
      Correctsourcelocation.value = true;
      CorrectproductId.value = true;
     console.log(props.initialData.id);
    const selectedData = jsonData.find(item => item.id === props.initialData.id);
    console.log(selectedData);
    if (selectedData) {
      // Accessing values from the selected data
      const jsonTheme = selectedData.metadata.theme;
      const jsonSubtheme = selectedData.metadata.subtheme;
      const jsonProductId = selectedData.metadata.product_id; // Assuming productId is the id property in the JSON
       const jsonSourceLocation = selectedData.source_location;
      // Setting initial values based on JSON data
      theme.value = jsonTheme;
      subtheme.value = jsonSubtheme;
      productId.value = jsonProductId;
       sourcelocation.value = jsonSourceLocation; // Check if source_location is defined before joining
      console.log("this is src location ",sourcelocation.value);
    } else {
    ...
    const handleRightClick = () => {
      ...
      console.log("emiting");
      console.log("src location aray:" + sourceLocationArray.value )
