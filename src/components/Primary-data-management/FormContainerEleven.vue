...
export default {
 ...
 watch(() => props.initialData, (newVal, oldVal) => {
      if (newVal) {
        ...
        pd.value = newVal.pd;
        accReport.value = newVal.accReport;
        horpor.value = newVal.horpor;
    ...
    if(props.initialData && props.initialData.id){
     ...
     CorrectAccR.value = true;
     Correcthorpor.value = true;
     Correctpd.value = true;
    const selectedData = jsonData.find(item => item.id === props.initialData.id);
    if (selectedData) {
      // Accessing values from the selected data
      const jsonPd = selectedData.metadata.pd;
      const jsonAccReport = selectedData.metadata.accReport;
      const jsonHorPor = selectedData.metadata.horpor;
      
      // Setting initial values based on JSON data
      pd.value = jsonPd;
      accReport.value = jsonAccReport;
      horpor.value = jsonHorPor;
      
      
      console.log("horpor:," , horpor.value);
    } else {
