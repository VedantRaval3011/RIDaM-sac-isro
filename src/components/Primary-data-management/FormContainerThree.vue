   ...
   setup(props, { emit }) {
    ...
    watch(() => props.initialData, (newVal, oldVal) => {
      if (newVal) {
        ...
        ContactPerson.value = newVal.ContactPerson;
        organization.value = newVal.organization;
        MailAddress.value = newVal.MailAddress;
        ContactTelephone.value = newVal.ContactTelephone;
        FaxNumber.value = newVal.FaxNumber;
        email.value = newVal.email;
      }
    }, { immediate: true });

    if(props.initialData && props.initialData.id){
     CorrectContactPerson.value = true; 
     CorrectContactTelephone.value = true; 
     CorrectEmail.value = true; 
     CorrectFaxNumber.value = true; 
     CorrectMailAddress.value = true; 
     CorrectOrganization.value = true; 
    const selectedData = jsonData.find(item => item.id === props.initialData.id);
    if (selectedData) {
      // Accessing values from the selected data
      const jsonCountry = selectedData.metadata.country;
      const jsonCity = selectedData.metadata.city;
      const jsonContactPerson = selectedData.metadata.contact_person; 
      const jsonOrganization =selectedData.metadata.organization;
      const jsonMailingAddress = selectedData.metadata.mailng_address;
      const jsonContactTelephone = selectedData.metadata.contact_telephone;
      const jsonContactFax = selectedData.metadata.contact_fax;
      const jsonEmail = selectedData.metadata.contact_email;
      // Setting initial values based on JSON data
      country.value = jsonCountry;
      city.value = jsonCity;
      ContactPerson.value = jsonContactPerson;
      organization.value = jsonOrganization;
      MailAddress.value = jsonMailingAddress;
      ContactTelephone.value = jsonContactTelephone;
      FaxNumber.value = jsonContactFax;
      email.value = jsonEmail;
