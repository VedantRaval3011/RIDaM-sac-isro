  ...
  setup(props, { emit }) {
    ...
    watch(() => props.initialData, (newVal, oldVal) => {
      if (newVal) {
        ...
        spheroid.value = newVal.spheroid;
        datum.value = newVal.datum;
       
      }
    }, { immediate: true });

    if(props.initialData && props.initialData.id){
     CorrectDatum.value = true;
     CorrectSpheroid.value = true;
    const selectedData = jsonData.find(item => item.id === props.initialData.id);
    if (selectedData) {
      // Accessing values from the selected data
      const jsonSpheroid = selectedData.metadata.spheroid;
      const jsonDatum = selectedData.metadata.datum;
      
      // Setting initial values based on JSON data
      spheroid.value = jsonSpheroid;
      datum.value = jsonDatum;
